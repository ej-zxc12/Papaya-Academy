...
function AboutPageContent() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(orgChartQuery, (snapshot) => {
      ...
      // Sort members by grade level (not by order field)
      const gradeOrder = ['Kinder Adviser', 'Grade 1 Adviser', 'Grade 2 Adviser', 'Grade 3 Adviser', 'Grade 4 Adviser', 'Grade 5 Adviser', 'Grade 6 Adviser', 'Registrar', 'Science / Registrar'];
      
      const getGradeIndex = (role: string) => {
        const index = gradeOrder.findIndex(g => role.includes(g));
        return index === -1 ? 999 : index;
      };

      data.academicStaff.sort((a, b) => getGradeIndex(a.role) - getGradeIndex(b.role));
      data.gradeAdvisers.sort((a, b) => getGradeIndex(a.role) - getGradeIndex(b.role));
                ...
                <>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 mb-8">
                    {(() => {
                      ...
                      const sortedStaff = [...orgChartData.academicStaff].sort((a, b) => getGradeIndex(a.role) - getGradeIndex(b.role));
                      return sortedStaff.slice(0, 4).map((person, index) => (
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 mb-16">
                    {(() => {
                      ...
                      const sortedStaff = [...orgChartData.academicStaff].sort((a, b) => getGradeIndex(a.role) - getGradeIndex(b.role));
                      return sortedStaff.slice(4).map((person, index) => (
                        <ScrollReveal key={person.id} animation="fade-up" delay={100 + index * 50}>
                          <div className="flex flex-col items-center">
                            <div className="w-24 h-24 rounded-full overflow-hidden border-3 border-papaya-green mb-4 shadow-md bg-gray-100">
                              <img
